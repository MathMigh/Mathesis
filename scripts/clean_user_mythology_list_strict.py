...
OUT_CLEAN = OUT_DIR / "Lista_de_mitologias_organizada_v3_pedra.txt"
OUT_REMOVED = OUT_DIR / "Lista_de_mitologias_organizada_v3_pedra_removidos.txt"
...
EXACT_REMOVE = {
    ...
    "Guerra Ática",
    "Guerra dos Sete Chefes contra Tebas",
    "Ciclo mitológico irlandęs",
...
def should_remove(line: str) -> bool:
    ...
    if any(token in stripped for token in CONTAINS_REMOVE):
        return True
    if any(re.search(pattern, stripped, re.I) for pattern in REGEX_REMOVE):
        return True
    return False
...
def main() -> None:
    ...
    OUT_REMOVED.write_text(
        "Itens removidos na peneira rígida\n\n"
